...
class ReviewBudget:
    def __init__(self):
        ...
        self.data=json.loads(self.path.read_text(encoding="utf-8")) if self.path.exists() else {"max_calls":8000,"max_usd":15.0,"attempts":[]}
...
def call(model,sample,key,budget,run):
    ...
    try:
        if config.get("endpoint")=="tencent":
            # 腾讯代理有 5 小时 1200 次窗口；用满后自动切官网端点
            if _ds_official_only[0]:
                url="https://api.deepseek.com/chat/completions";auth=os.environ["JUDGE_DS_OFFICIAL_KEY"];body["model"]="deepseek-v4-flash"
            else:
                url=os.environ["JUDGE_DS_BASE_URL"].rstrip("/")+"/chat/completions";auth=os.environ["JUDGE_DS_KEY"]
            body.pop("provider",None);body.pop("usage",None);body.pop("reasoning",None)
            body["temperature"]=0;body["thinking"]={"type":"disabled"}
            body["response_format"]={"type":"json_object"}
            body["messages"][0]["content"]=RUBRIC+"\n\nReturn a single JSON object matching this schema exactly:\n"+json.dumps(schema(),ensure_ascii=False)
        else:
            url="https://openrouter.ai/api/v1/chat/completions";auth=key
        r=requests.post(url,headers={"Authorization":"Bearer "+auth},json=body,timeout=75,allow_redirects=False)
        if r.status_code!=200:
            result["error"]="HTTP "+str(r.status_code);result["provider_error"]=r.text[:1000]
            if config.get("endpoint")=="tencent" and not _ds_official_only[0] and (r.status_code in (429,402,403) or "quota" in r.text.lower() or "limit" in r.text.lower()):
                _ds_official_only[0]=True
        else:
...
_ds_official_only=[False]
_RPM = {"luna": 18, "qwen": 60, "gemini": 60, "dsv4": 120}
_gate = {}
_gate_lock = threading.Lock()
def _throttle(model):
    """Keep each judge under its account rate limit instead of burning calls on 429s."""
    rpm = _RPM.get(model, 60); gap = 60.0 / rpm
    with _gate_lock:
        prev = _gate.get(model, 0.0)
        now = time.monotonic()
        start = max(now, prev)
        _gate[model] = start + gap
...
def run_review(name,samples,models,env):
    ...
    old=rows(out/"raw.jsonl");done={(r["model"],r["sample_id"]) for r in old if r.get("rating") and not r.get("error")};budget=ReviewBudget();key=keys(env)["OPENROUTER_API_KEY"]
    ...
    def work(job):
        ...
        for attempt in range(6):
            _throttle(model)
            result=call(*job,key,budget,name)
            if result.get("error")!="HTTP 429": break
            time.sleep(2*(attempt+1))
        append(out/"raw.jsonl",result);return result
    with concurrent.futures.ThreadPoolExecutor(max_workers=int(os.environ.get('JUDGE_CONCURRENCY','8'))) as pool:
