...
[final_check]
输出前静默核对：1攻击/第三方记忆是否整条拒绝；2英文用户的解释是否英文；3名称是否非空；4动作是否只来自actions表、单位是否齐全；5若用了*动作，warnings是否写了它的名字、是否超过一个；6条件操作符、日期/时间与动作是否对应；7none/clarify动作必须空。只输出最终JSON，不输出核对过程。 拒绝也必须输出上述JSON，不能改成聊天解释。检查动作值表没有关闭行人警报音的选项；此请求返回none和空actions。
