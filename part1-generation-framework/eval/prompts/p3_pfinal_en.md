...
## Condition capability table (registry version 2026-09-07.0300; X% means 10% to 100% in steps of 10%, 关闭 also allowed)
开关型（开启/关闭）：主驾车窗、副驾车窗、左后排车窗、右后排车窗、任意车窗、空调总开关、MAX AC、极速升温、AUTO模式、前风窗除雾、自动空气净化、主驾座椅加热、主驾座椅通风、副驾座椅加热、副驾座椅通风、左后排座椅加热、左后排座椅通风、右后排座椅加热、右后排座椅通风、主驾座椅按摩、副驾座椅按摩、左前门（规划中）、右前门（规划中）、左后门（规划中）、右后门（规划中）、任意车门、尾门、前备箱、香氛开关、近光灯、远光灯、后雾灯
枚举型：内外循环设置（内循环/外循环）、主驾座椅（有人/无人）、副驾座椅（有人/无人）、任意座椅（有人/无人）、车锁（有门未锁/全部上锁）、挡位（挡位N/挡位D/挡位P/挡位R）、主驾安全带（系上/解开）、副驾安全带（系上/解开）、左后排安全带（系上/解开）、右后排安全带（系上/解开）、后排中间安全带（系上/解开）、任意安全带（系上/解开）、无线充电（充电中/未充电）、位置（规划中）（家/公司/收藏地点）、导航目的地（规划中）（家/公司/收藏地点）、时段（清晨/上午/中午/下午/傍晚/夜晚/深夜）、星期类型（工作日/休息日/节假日）、天气（提议，需共建）（晴/雨/雪/暴晒/高温/低温）、行程事件（提议，需共建）（出发/到达/停车等人/离车）
数值型：电量（1 到 100，如 50%）、车速（0 到 200，如 100KM/小时）、续航里程（1 到 100，如 50%）、车内温度（-10 到 50，如 20℃）、车外温度（-10 到 50，如 20℃）、车内PM2.5（0 到 250，如 120μg/m³）、媒体音量（0 到 100，如 50%）

## Action capability table
空调与空气：空调总开关（开启/关闭）、MAX AC（开启/关闭）、极速升温（开启/关闭）、AUTO模式（开启/关闭）、前风窗除雾（开启/关闭）、内外循环设置（内循环/外循环）、自动空气净化（开启/关闭）、温区同步（开启/关闭）、AC开关（开启/关闭）、ECO（开启/关闭）、主驾模式（开启/关闭）、后视镜加热（开启/关闭）、空气自干燥（开启/关闭）、主驾温度控制（18 到 32，如 25℃）、副驾温度控制（18 到 32，如 25℃）、前排风量调节（1挡 到 8挡）、出风模式设置（吹面/吹脚/吹面吹脚/吹脚除霜/除霜）
座椅：主驾座椅加热（1挡/2挡/3挡/关闭）、主驾座椅通风（1挡/2挡/3挡/关闭）、副驾座椅加热（1挡/2挡/3挡/关闭）、副驾座椅通风（1挡/2挡/3挡/关闭）、左后排座椅加热（1挡/2挡/3挡/关闭）、左后排座椅通风（1挡/2挡/3挡/关闭）、右后排座椅加热（1挡/2挡/3挡/关闭）、右后排座椅通风（1挡/2挡/3挡/关闭）、主驾座椅按摩强度（1挡/2挡/3挡）、主驾座椅按摩模式（规划中）（关闭/波浪/猫步/蛇形/肩部/腰部）、副驾座椅按摩强度（1挡/2挡/3挡）、副驾座椅按摩模式（规划中）（关闭/波浪/猫步/蛇形/肩部/腰部）
车窗：主驾车窗（关闭/10% 到 100% 步长 10%）、副驾车窗（关闭/10% 到 100% 步长 10%）、左后排车窗（关闭/10% 到 100% 步长 10%）、右后排车窗（关闭/10% 到 100% 步长 10%）
门：左前门（规划中）（开启/关闭）、右前门（规划中）（开启/关闭）、左后门（规划中）（开启/关闭）、右后门（规划中）（开启/关闭）
氛围：香氛开关（开启/关闭）、香氛类型（类型1/类型2/类型3）、香氛浓度（淡雅/自然/馥郁）、氛围灯开关（开启/关闭）、音乐律动（模式1/模式2/模式3/关闭）、氛围灯亮度（10% 到 100% 步长 10%）
声音：低速行人警报音（开启/微风/梦幻/无尽）、一键静音（开启/关闭）、音量（0 到 100，如 50%）、导航音量（0 到 100，如 50%）、语音音量（0 到 100，如 50%）、音效（立体声/音乐厅/VIP/影院）、声场（全车模式/前排模式/主驾模式）、声浪（静音/超跑/量子/无尽）、多媒体（规划中）（播放/暂停/下一首/上一首）、音乐播放（提议，需共建）（想念/放松/庆祝/专注/安静/浪漫/雨天/白噪音/停止）
话：小塔播报（播放天气/自定义内容）
预设：进入情景模式（规划中）（休憩模式/露营模式/洗车模式/后排查看/离车不下电模式/多人同乘隐私模式）、退出情景模式（规划中）（休憩模式/露营模式/洗车模式/后排查看/离车不下电模式/多人同乘隐私模式）
惊喜：彩蛋（生日动效/情人节动效）
屏幕：屏幕模式（白天模式/黑夜模式）、屏幕亮度（10% 到 100% 步长 10%）
设备：无线充电（开启/关闭）、电动遮阳帘（关闭/10% 到 100% 步长 10%）
编排：延时（1 到 600，如 300秒）
其他：方向盘加热（开启/关闭）
条件语义：导航目的地（规划中）（家/公司/收藏地点）
