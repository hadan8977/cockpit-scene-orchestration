...
def score_item(item, obj, prompt_style):
    res = {"json_valid": obj is not None, "schema_valid": False, "pass": False, "violations": [], "alt_hit": None,
           "fail_reason": "", "extras": 0, "name_ok": None, "intent_derived": None, "intent_ok": None, "collapse_exact": None, "collapse_loose": None}
    if obj is None:
    ...
    res["name_ok"] = 0 < len(out["name"]) <= 10 if out["name"] else False
...
def call_model(cfg, system_prompt, user_msg):
    ...
    t0 = time.time(); ttft = None; ttfr = None; content = []; reasoning_chars = 0; usage = None; err = None
    try:
        with requests.post(url, headers=headers, json=body, stream=True, timeout=cfg["timeout"]) as r:
            if r.status_code != 200:
                return {"text": None, "error": "HTTP %d %s" % (r.status_code, r.text[:300]), "latency": time.time() - t0, "ttft": None, "ttfr": None, "usage": None, "reasoning_chars": 0}
        ...
        err = str(e)
    return {"text": "".join(content) if content else None, "error": err, "latency": time.time() - t0, "ttft": ttft, "ttfr": ttfr,
...
def mock_output(item, style, noise=False):
    ...
    if style == "p3":
        obj.update({"relevance": 0.7 if intent in ("affect", "vague") else (0.0 if intent == "none" else 0.9),
                    "understanding": "模拟理解，引用“%s”" % item["input"][:6] if intent not in ("none",) else "", "memory": []})
    return obj
...
CAT_ORDER = ["action", "precise", "vague", "affect", "robust", "attack"]
...
def main():
    ...
    if live and not cfg["api_key"]:
        sys.exit("缺少 EVAL_API_KEY（或用 --mock 离线自检）")
    tag = args.tag or (("mock-" + style + ("-noise" if args.mock_noise else "")) if not live else datetime.now().strftime("%Y%m%d-%H%M%S") + "-" + cfg["model"] + "-" + style)
    ...
    def run_one(job):
        ...
        if live:
            ...
            r = {"text": json.dumps(obj, ensure_ascii=False), "error": None, "latency": 0.0, "ttft": 0.0, "ttfr": None, "usage": None, "reasoning_chars": 0}
        sc = score_item(it, obj, style)
        return {"id": it["id"], "cat": it["cat"], "lang": lang, "rep": rep, "input": inp, "tests": it["tests"], "raw_text": r["text"], "error": r["error"],
                "latency": r["latency"], "ttft": r.get("ttft"), "ttfr": r.get("ttfr"), "usage": r["usage"], "reasoning_chars": r["reasoning_chars"], "score": sc}
    rows = []
    ...
    summary = {"tag": tag, "model": cfg["model"] if live else "mock", "prompt": os.path.basename(prompt_path), "style": style,
               "thinking": cfg["thinking"] if live else None, "json_mode": cfg["json_mode"] if live else None, "langs": langs,
    ...
    L = ["# 评测结果 %s" % tag, "", "模型：%s；prompt：%s；schema 风格：%s；思考：%s；json 模式：%s；语言：%s；题数 %d × 重复 %d" % (
            summary["model"], summary["prompt"], style, summary["thinking"], summary["json_mode"], "+".join(langs), len(items), args.repeat), "",
         "| 指标 | 值 |", "|---|---|",
